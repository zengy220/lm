...
<div id="content-header">
  <div id="breadcrumb"> <a href="javascript:;" class="tip-bottom">广告位设置</a><a href="/<?php echo (MODULE_NAME); ?>/Advertise/ad_content_list" class="current">广告内容</a></div>
</div>
  ...
  <div class="row-fluid">
	...
	<div class="span11">
        <div class="form-group">
            <form method="post" action="/<?php echo (MODULE_NAME); ?>/Advertise/ad_content_list">

                <select style="width:150px;"  class="form-control" name="ad_list_id">
                    广告位筛选<option value="">请选择</option>
                    <?php if(is_array($ad_list)): foreach($ad_list as $key=>$vo): ?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo["id"] == $ad_list_id): ?>selected='selected'"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; ?>
                </select>&nbsp;&nbsp;&nbsp;

                &nbsp;&nbsp;&nbsp;
                <button type="submit" class="btn btn-success">查询</button>
                <a href="/<?php echo (MODULE_NAME); ?>/Advertise/ad_content_add" class="btn btn-success btn" style="float: right;">新增广告内容</a>
     ...
     <div class="widget-box">
          ...
          <div class="widget-content ">
            <table class="table table-bordered table-striped with-check">
			 ...
			 <?php if(is_array($ad_content_list)): $i = 0; $__LIST__ = $ad_content_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vs): $mod = ($i % 2 );++$i;?><tr>
                  <td style="text-align:center;"><?php echo ($i); ?></td>
				  <td style="text-align:center;"><?php echo ($vs["title"]); ?></td>
				  <td style="text-align:center;"><?php echo ($vs["sort"]); ?></td>
                  <td style="text-align:center;"><?php echo ($vs["url"]); ?></td>
				  <td style="text-align:center;">
					<a href="/<?php echo (MODULE_NAME); ?>/Advertise/ad_content_edit/ad_content_id/<?php echo ($vs["id"]); ?>" class="btn btn-primary btn-mini">修改</a> |
					 <a href="/<?php echo (MODULE_NAME); ?>/Advertise/ad_content_del/ad_content_id/<?php echo ($vs["id"]); ?>" class="btn btn-danger btn-mini" onclick= "if(confirm('是否确定删除 <?php echo ($vs["title"]); ?>!')==false)return  false; ">删除</a>
				  	</if>
				  </td>	  
